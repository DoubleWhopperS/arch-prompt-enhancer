...
当【设计意图】中包含 \`[摄影模式]\` 标记时（"室外建筑摄影"或"室内建筑摄影"风格触发），进入**摄影模式**——目的是让 Nano Banana Pro 调用真摄影分布而非渲染分布。

**核心理念**：扩散模型听不懂"Phase One IQ4 / Capture One Pro / ISO 100"等摄影师工作流术语，但听得懂"玻璃有指纹和灰尘"、"水面有微涟漪不是镜面"、"画面四角自然渐暗"等**具体视觉描述**。摄影模式 prompt 已用全中文视觉描述重写，此分支负责保护它们不被剥离。

摄影模式下默认规则调整：
- 规则 2 不适用：必须保留"真实摄影作品（非 CG 渲染）"等类别切换标记
- 规则 4 不适用：必须保留风格锚点（摄影师名字是模型识别的强 token）。**风格锚点处理规则**：风格 prompt 中提供了一个按场景分类的摄影师池，必须根据当前用户场景（建筑类型/材质/风格/氛围）从池中选择 **2-3 位风格匹配**的摄影师作为锚点输出。**禁止只挑第一位 Iwan Baan**（虽然知名度高，但单一化会让所有输出风格趋同）。例如：清水混凝土极简空间应选 Hélène Binet + Eiichi Kano；工业钢结构应选 Roland Halbe + Edmund Sumner；中国当代项目应选 苏圣亮 + 田方方；公共建筑通用场景才用 Iwan Baan + Hufton+Crow。
- 规则 6 例外：基础图建筑几何不重述（保持），但风格 prompt 中的具体视觉描述（玻璃指纹/水面非镜面/灯具暖色光晕/胶片颗粒/镜头暗角/真实瑕疵等）必须**完整保留**，不可软化为模糊氛围词
- 规则 7 不适用：可保留"长曝快门"、"运动模糊"等成像工艺描述
- 规则 8 字数上限放宽到 **350 字**（不超过 400 字）

### 摄影模式输出结构（两段，无标题）

**段 1 — 视觉描述（150-220 字）**：
将风格 prompt 中的视觉描述细节融入用户场景，明确点出"真实摄影作品，不是 CG 渲染"。包含：光线特征 / 材质表现（玻璃/混凝土/金属/木质等的真实瑕疵） / 水面或地面细节（菲涅尔效应/磨损/水渍） / 灯具表现（室内：暖色光晕） / 植被状态（不对称生长/枯枝） / 噪点与暗角 / 风格锚点（摄影师名字）。**允许列表化或半列表化呈现，不必强行散文化**。

**段 2 — 场景增量（80-130 字）**：
人物（姿态/服饰/数量/运动模糊状态）+ 真实瑕疵（地面磨损/玻璃指纹/植被枯枝/施工纹路）+ 用户意图中的场景特定要素。**不复述基础图的建筑几何/立面/空间布局**——这部分由基础图本身承载。

### 摄影模式注意事项

- 输出最终 prompt 时**移除** \`[摄影模式]\` sentinel 标记，不要让它出现在生图 prompt 里
- 风格 prompt 中的视觉描述必须**忠实保留**，不可软化、不可略写为模糊氛围词
- "克制用词"指南调整为"具体视觉化"：避免"商业大片质感"、"电影感"、"高保真"等空洞形容词，要写出具体可视化的物理细节（"玻璃有手指印和灰尘"而非"玻璃质感真实"）

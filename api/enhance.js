...
当【设计意图】中包含 \`[摄影模式]\` 标记时（"室外建筑摄影"或"室内建筑摄影"风格触发），进入**摄影模式**——目的是让 Nano Banana Pro 调用真摄影分布而非渲染分布。以下默认规则**在摄影模式下不适用**：

- 规则 2 不适用：必须保留"真实建筑摄影作品（非渲染图）"等类别切换标记
- 规则 4 不适用：必须保留预设工作流术语（Capture One Pro / Photomatix Pro / X-Rite ColorChecker / HDR ±2EV 包围合成 / 反 HDR 映射 / Highlights ≥-70 / Shadows ≤+50 / Vibrance > Saturation 等）
- 规则 7 不适用：必须保留相机器材与参数（Phase One IQ4 150MP / Hasselblad H6D-100c / Sony A7R V / Canon TS-E 17mm f/4L 移轴 / Sigma 14-24mm / ISO 100-800 / f/8-f/11 / 三脚架长曝 1/4-2s / 反光镜预升 / HDR 包围曝光 / 移轴 ±12mm shift 等）
- 规则 8 字数上限放宽到 **350 字**（不超过 400 字）

### 摄影模式输出结构（两段，无标题）

**段 1 — 摄影规格（150-220 字）**：
列出器材选型 + 关键参数 + 后期工作流 + 物理正确性约束（菲涅尔反射 30-60% / 大气透视 -30% contrast / ISO X 下 X% luminance noise / 镜头四角 -0.3 至 -0.5EV 暗角等）+ 反 CG 禁忌（零噪点 / AO 重 / 玻璃完美 / 灯具完美阵列 / 窗外死白等）。允许列表化或半列表化呈现，不必强行散文化。

**段 2 — 场景增量（80-130 字）**：
人物（姿态/位置/数量/服饰）+ 真实瑕疵（地面磨损 / 玻璃污渍 / 植被枯枝 / 施工纹路 / 椅子轻微不齐）+ 用户意图中提及的场景特定要素。**不复述基础图的建筑几何/立面/空间布局**——这部分由基础图本身承载。

### 摄影模式注意事项

- 输出最终 prompt 时**移除** \`[摄影模式]\` sentinel 标记，不要让它出现在生图 prompt 里
- 风格 prompt 中的 know-how 必须**完整保留并融入段 1**，不可剥离或软化
- 摄影模式下"克制用词"指南调整为"克制于事实"：可以用"必须"、"严格"、"明确禁止"等强语气，但围绕器材/参数/物理事实而非夸饰

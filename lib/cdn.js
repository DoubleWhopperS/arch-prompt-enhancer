...
async function uploadBufferToCDN(buffer, ext) {
  // 通道 1: R2 presign（返回公网 CDN URL，浏览器可直接加载）
  const r2Token = process.env.TENSORART_BEARER_TOKEN;
  if (r2Token) {
    try {
      return await r2PresignUpload(buffer, ext, r2Token);
    } catch (err) {
      console.warn(`[cdn] R2 presign 上传失败: ${err.message}`);
    }
  }

  // 通道 2: TAMS OSS（返回 resource_id，仅 TAMS API 可解析，浏览器不能直接加载）
  // 当 R2 不可用时作为降级，返回 resource_id 格式 URL
  const tamsToken = process.env.TAMS_TOKEN || process.env.TAMS_API_TOKEN;
  if (tamsToken) {
    const { resourceId } = await tamsOssUpload(buffer, tamsToken);
    // 注意：此 URL 依赖 TAMS 网关代理，浏览器直接访问可能无效
    return `${TAMS_BASE_URL}${TAMS_RESOURCE_ENDPOINT}/${resourceId}`;
  }

  throw new Error('图片上传不可用：TENSORART_BEARER_TOKEN 和 TAMS_TOKEN 均未配置');
}

/**
 * 上传 Buffer 到 TAMS OSS，返回 resource_id
 * 仅用于 TAMS API 调用（如视频生成），不返回公网 URL
 *
 * @param {Buffer} buffer
 * @returns {Promise<string>} resource_id (UUID)
 */
async function uploadBufferToTAMS(buffer) {
  const token = process.env.TAMS_TOKEN || process.env.TAMS_API_TOKEN;
  if (!token) throw new Error('TAMS_TOKEN 未配置');
  const { resourceId } = await tamsOssUpload(buffer, token);
  return resourceId;
}
